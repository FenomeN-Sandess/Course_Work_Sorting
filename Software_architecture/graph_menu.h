...
protected:
	...
	float position_message_y = windowMessageHeight * 0.33f; // Позиция сообщения по оси OY
	float position_agreement_y = windowMessageHeight * 0.7f; // Позиции ok по оси OY
	float position_rectangle_y = windowMessageHeight * 0.6f;

	unsigned int size_font = static_cast<unsigned int>(windowWidth / 15); // Размер кнопок в меню
	unsigned int size_font_message = static_cast<unsigned int>(windowMessageWidth / 20); // Размер кнопок в диалоговом окне
	float rectangle_outline_thickness = 2;
	float border_size_button = 3; // Размер границ у кнопок в главном меню
	float border_size_button_window = 1; // Размер границ у кнопок в диалоговом окне
	std::vector <float> rectangle_window_size = { static_cast<float>(size_font * 2), static_cast<float>(size_font*1.5) };

	sf::Color rectangle_outline_color = sf::Color::Black;
	...
	void define_center_position_x(sf::Text& text, unsigned int& width); // Установить sf::Text по центру
	void define_center_position_x(sf::RectangleShape& rectangle, unsigned int& width); // Установить прямоугольник по центру
	void define_center_position_y(sf::Text& text, unsigned int& height);
	void define_center_position_y(sf::RectangleShape& rectangle, unsigned int& height);
	void define_center_position_area(sf::Text& text, sf::RectangleShape& rectangle);
	void define_center_position_area(sf::RenderWindow& window, sf::Vector2i& external_window_position, sf::Vector2i& external_window_size);
	void define_center_position_area(sf::Text& text, unsigned int window_width, unsigned int window_height);
