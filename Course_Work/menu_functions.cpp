...
#include <functional> // Библиотека C++, использующая для представления метода класса как обычной функции
...
void menu::functions::save_time(std::function<void(std::vector<double>)> view_sorting, std::vector<std::vector<double>> &view_time, int n)
{
...
void menu::functions::sorting()
{
	for (int n = 0; n <= 10; n++) {
		...
		save_time(std::bind(&sorting::bubble_sorting, &Sort, std::placeholders::_1), Info.bubble_time, n);
		save_time(std::bind(&sorting::pyramid_sorting, &Sort, std::placeholders::_1), Info.pyramid_time, n);
...
void menu::functions::display()
{
	...
	//window.setFramerateLimit(60); // Ограничение частоты кадров до 60 кадров в секунду
